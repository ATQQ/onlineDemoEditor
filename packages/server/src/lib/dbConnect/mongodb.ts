...
const { host, port, database } = mongodbConfig

const url = `mongodb://${host}:${port}/${database}?wtimeoutMS=2000`
...
export function query<T>(callback: Callback<T>): Promise<T> {
  const p = new Promise<T>((resolve, rej) => {
    getDBConnection().then(({ db, Db: _Db }) => {
      // 执行回调
      callback(_Db, resolve)
      // resolve后关闭
...
export function updateCollection<T>(
  collection: string,
  _query: FilterQuery<T>,
  data: UpdateQuery<T>,
  many = false
) {
  return mongoDbQuery<UpdateWriteOpResult>((db, resolve) => {
    if (many) {
      db.collection<T>(collection).updateMany(_query, data).then(resolve)
      return
    }
    db.collection<T>(collection).updateOne(_query, data).then(resolve)
  })
}
