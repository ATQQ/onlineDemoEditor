<template>
    <div class="editor-container">
        ...
        <div class="other-editor">
            <div v-show="activeEditor === 'CSS'" id="css-editor">

            </div>
            <div v-show="activeEditor === 'JS'" id="js-editor">
...
onMounted(() => {
    const delay = 200
    // 从接口拿初始化数据
    htmlEditor.value = monaco.editor.create(document.getElementById('html-editor')!, {
    ...
    CSSEditor.value = monaco.editor.create(document.getElementById('css-editor')!, {
        value: $codeStore.css,
        language: 'css',
        theme: 'vs-dark',
        fontSize: 18,
    ...
    toRaw(htmlEditor.value).onDidChangeModelContent(debounce(() => {
        $codeStore.updateHtml(toRaw(htmlEditor.value).getValue())
    },delay))
    toRaw(CSSEditor.value).onDidChangeModelContent(debounce(() => {
        $codeStore.updateCss(toRaw(CSSEditor.value).getValue())
    },delay))
    toRaw(JSEditor.value).onDidChangeModelContent(debounce(() => {
